...
import org.openepics.discs.calib.ent.PreferenceName;
...
public class PreferencesEJB {
     ...
     * @return preferences
     */
    public List<UserPreference> findPreferences(AuthUser user) {
     ...
     */
    public UserPreference findPreference(AuthUser user, PreferenceName name) {
        List<UserPreference> prefs;
    ...
    /**
     * Find the preference values of a given preference name of a user (for
     ...
     */
    public List<UserPreference> findPreferences(AuthUser user, PreferenceName name) {
        List<UserPreference> prefs;
        TypedQuery<UserPreference> query = em.createQuery("SELECT p FROM UserPreference p WHERE p.user = :user AND p.name = :name", UserPreference.class)
                .setParameter("user", user)
                .setParameter("name", name.toString());
     ...
     */
    public List<AuthUser> findUsers(PreferenceName name, String value) {
        List<AuthUser> users;
        TypedQuery<AuthUser> query = em.createQuery("SELECT p.user FROM UserPreference p WHERE p.prefValue = :value AND p.name = :name", AuthUser.class)
     ...
     * @param value
     */
    public void savePreference(AuthUser user, PreferenceName name, String value) {
     ...
     * @param values
     */
    public void savePreference(AuthUser user, PreferenceName name, List<String> values) {
    ...
    public void deletePreference(UserPreference pref) {
        UserPreference upref = em.find(UserPreference.class, pref.getId());
        if (upref != null) {
            em.remove(upref);
        }
    }
